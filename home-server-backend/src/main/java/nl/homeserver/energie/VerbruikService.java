...
public class VerbruikService {
    ...
    public List<VerbruikInUurOpDag> getVerbruikPerUurOpDag(final LocalDate day) {
        return IntStream.range(0, 24)
                        .mapToObj(hourOfDay -> getVerbruikInUur(day, hourOfDay))
                        .collect(toList());
    }

    public List<VerbruikInMaandInJaar> getVerbruikPerMaandInJaar(final Year year) {
        return allOf(Month.class).stream()
                                 .map(monthInYear -> getVerbruikInMaand(YearMonth.of(year.getValue(), monthInYear)))
                                 .collect(toList());
    }

    public List<VerbruikKostenOpDag> getVerbruikPerDag(final DatePeriod period) {
    ...
    public VerbruikKostenOverzicht getGemiddeldeVerbruikEnKostenInPeriode(final DatePeriod period) {
